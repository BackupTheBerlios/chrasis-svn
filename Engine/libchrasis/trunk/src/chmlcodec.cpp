...
chrasis::Character::collection
chrasis::read_chml(std::string ifilename)
{
	chrasis::Character::collection ret_chrs;

	try {
		xmlpp::TextReader reader(ifilename);

		// characters
		chrasis::Character tmp_char;
		while(reader.read())
		{
			...
			chrasis::Character tmp_char(reader.get_attribute("name"));
			// strokes
			while(reader.read())
			{
				if (reader.get_name() == "character")
...
void
chrasis::write_chml(chrasis::Character::collection & chrs, std::string ofilename)
{
	try
	{
		...
		for (chrasis::Character::iterator ci = chrs.begin();
		     ci != chrs.end();
		     ++ci)
		{
			...
			for (chrasis::Stroke::iterator si = ci->strokes_begin();
			     si != ci->strokes_end();
			     ++si)
			{
				     ...
				     ++pi)
				{
					...
					pNode->set_attribute("x", chrasis::toString(pi->x()));
					pNode->set_attribute("y", chrasis::toString(pi->y()));
				}
			}
		}
